...
router.delete('/tests/:id', auth(['admin','teacher']), onlineMCQTestController.deleteTest); // Delete a test by ID
router.get('/tests', auth(['admin','student','teacher']), onlineMCQTestController.getAllTests); // Get all tests
router.get('/tests/:testId', auth(['admin','student','teacher']), onlineMCQTestController.getTestById); // Get a test by ID
